// Customized copy of vuepress-theme-hope's BloggerInfo
// (lib/client/components/blog/BloggerInfo.js). Identical to the original
// except the `vp-blog-counts` row gains a series stat between 标签 and
// 时间轴, linking to /series/ with the count from seriesPlugin.
//
// Takes effect only with `custom: true` in theme.ts plus the alias
// registered in seriesPlugin.js.
...
import { getAuthor } from "vuepress-theme-hope/shared";
import { seriesMap } from "@temp/series/map.js";
import "@theme-hope/styles/blog/blogger-info.scss";
...
export default defineComponent({
  ...
  setup(_props, { slots }) {
    ...
    return () => {
      ...
      const series = routeLocale.value === "/en/" ? "Series" : "专题";
      const countItems = [
